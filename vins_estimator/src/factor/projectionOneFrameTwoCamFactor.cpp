 ...
 * @param[in] _pts_j        特征点在 cam_i 右目的归一化平面上的坐标
 * @param[in] _velocity_i   特征点在 cam_i 左目的像素运动速度
 * @param[in] _velocity_j   特征点在 cam_i 右目的像素运动速度 
...
bool ProjectionOneFrameTwoCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
  ...
  Eigen::Vector3d tic(parameters[0][0], parameters[0][1], parameters[0][2]);                        // t_bci
  Eigen::Quaterniond qic(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);   // R_bci
  // 由于 相机j是右目图像，因此 T_bc的值和左目不一样
  Eigen::Vector3d tic2(parameters[1][0], parameters[1][1], parameters[1][2]);                       // t_bcj
  Eigen::Quaterniond qic2(parameters[1][6], parameters[1][3], parameters[1][4], parameters[1][5]);  // R_bcj

  double inv_dep_i = parameters[2][0];  // 逆深度 inv_dep

  double td = parameters[3][0];         // 相机和IMU时间戳之差 td
  ...
  Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;                 // P_bi = R_bci * P_ci + t_bci
  // 获得 特征点在 imu_j 坐标系下的坐标 P_bj
  Eigen::Vector3d pts_imu_j = pts_imu_i;                                // 由于相机i和j是同一个相机，因此  P_bj = P_bi
  // 获得 特征点在 cam_j 坐标系下的坐标 P_cj
  Eigen::Vector3d pts_camera_j = qic2.inverse() * (pts_imu_j - tic2);   // P_cj =  R_bcj^T * P_bj - R_bcj^T * t_bcj = R_bcj^T * (P_bj - t_bcj)  估计值
  Eigen::Map<Eigen::Vector2d> residual(residuals);

#ifdef UNIT_SPHERE_ERROR
  residual = tangent_base * (pts_camera_j.normalized() - pts_j_td.normalized());
#else
  double dep_j = pts_camera_j.z();    // 估计 特征点在 cam_j 的深度
