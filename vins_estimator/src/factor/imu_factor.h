...
/**
 * @brief IMU因子：关联前后两帧IMU位姿和IMU预积分量
 *        残差：15维
 *        posei posej：7维 ----> 四元数旋转4维、平移3维  由相机位姿乘上外参获得
 *        imui imuj：9维 ----> 速度3维、ba 3维、bg 3维
 */
class IMUFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9> {
 public:
  ...
  /**
   * @brief 计算 IMU因子的 残差 和 Jacobian
   * 
   * @param[in] parameters    parameters[0~3]分别对应了4组优化变量的参数块
   * @param[out] residuals    IMU预积分残差
  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    if (jacobians) {
      // 获取预积分的误差递推函数中 PQV 关于 ba、bg 的 Jacobian
