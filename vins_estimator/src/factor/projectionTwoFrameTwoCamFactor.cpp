 ...
 * @param[in] _pts_j        特征点在 cam_j 右目的归一化平面上的坐标
 * @param[in] _velocity_i   特征点在 cam_i 左目的像素运动速度
 * @param[in] _velocity_j   特征点在 cam_j 右目的像素运动速度 
...
bool ProjectionTwoFrameTwoCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
  ...
  Eigen::Vector3d tic(parameters[2][0], parameters[2][1], parameters[2][2]);                        // t_bci
  Eigen::Quaterniond qic(parameters[2][6], parameters[2][3], parameters[2][4], parameters[2][5]);   // R_bci
  // 由于 相机j是右目图像，因此 T_bc的值和左目不一样
  Eigen::Vector3d tic2(parameters[3][0], parameters[3][1], parameters[3][2]);                       // t_bcj
  Eigen::Quaterniond qic2(parameters[3][6], parameters[3][3], parameters[3][4], parameters[3][5]);  // R_bcj

  double inv_dep_i = parameters[4][0];    // 逆深度 inv_dep

  double td = parameters[5][0];           // 相机和IMU时间戳之差 td
